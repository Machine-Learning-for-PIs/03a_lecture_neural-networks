...
\node[activation] (a3) at (7*\horispace,0*\vertspace) {$f_3(\bar{\mathbf{h}}_3) = \mathbf{h}$};
\node (n3label) at (6*\horispace,-1*\vertspace) {Dense 3};
\node (a3label) at (7*\horispace,-1*\vertspace) {Activation function 3};

\node[output] (out) at (8*\horispace, 0*\vertspace) {$\mathbf{h}$}; 
\node (outlabel) at (8*\horispace,-1*\vertspace) {Output};
